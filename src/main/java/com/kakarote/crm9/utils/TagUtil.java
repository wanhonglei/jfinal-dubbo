...
public class TagUtil{
    ...
    public static Set<Integer> toSet(String tagStr){
        ...
        String[] split = tagStr.split(String.valueOf(SEPARATOR));
        for (String s : split) {
            try {
    ...
    public static String fromString(String tagStr){
        ...
        if(!tagStr.substring(0,1).equals(String.valueOf(SEPARATOR))){
            sb.append(SEPARATOR);
        }
        sb.append(tagStr);
        if(!tagStr.substring(tagStr.length()-1).equals(String.valueOf(SEPARATOR))){
            sb.append(SEPARATOR);
        }
        return sb.toString();
    }
