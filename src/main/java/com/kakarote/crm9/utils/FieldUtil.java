...
public class FieldUtil {
    ...
    public <T> List<Record> getFixedField(List<Record> fieldList, String fieldName, String name, T value, String formType,
                                      Object[] settingArr, Integer isNull,String tooltipContent) {
        Record record = new Record();
        record.set("name", name).set("value", value).set("field_name", fieldName).set("form_type", formType).set("field_type", 1)
                .set("setting", settingArr).set("is_null", isNull).set("tooltipContent", tooltipContent);
        fieldList.add(record);
        return fieldList;
    }
    ...
    public List<Record> getFixedFieldWithFieldName(String name, String fieldNameInDb, String value) {
        Record record = new Record();
        record.set("name",name).set("value",value == null ? "" : value).set("fieldName", fieldNameInDb);
        recordList.add(record);
        return recordList;
    }

    public FieldUtil set(String fieldName, String value){
        getFixedField(fieldName, value == null ? "" : value);
        return this;
        }

    public FieldUtil set(String fieldName, String fieldNameInDb, String value) {
        getFixedFieldWithFieldName(fieldName, fieldNameInDb, value);
        return this;
    }
