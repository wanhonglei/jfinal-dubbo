...
public class CrmProductService {
    ...
    public R saveAndUpdate(JSONObject jsonObject,Long userId) {
        ...
        if (crmProduct.getProductId() == null) {
            ...
            crmProduct.setCreateUserId(userId == null ? null:userId.intValue());
            crmProduct.setCreateTime(DateUtil.date());
            crmProduct.setUpdateTime(DateUtil.date());
            crmProduct.setOwnerUserId(userId == null ? null:userId.intValue());
            crmProduct.setBatchId(batchId);
            boolean save = crmProduct.save();
            crmRecordService.addRecord(crmProduct.getProductId().intValue(), CrmEnum.PRODUCT_TYPE_KEY.getTypes(),userId);
            return save ? R.ok() : R.error();
        } else {
            CrmProduct oldCrmProduct = new CrmProduct().dao().findById(crmProduct.getProductId());
            crmRecordService.updateRecord(oldCrmProduct, crmProduct, CrmEnum.PRODUCT_TYPE_KEY.getTypes(),userId);
            crmProduct.setUpdateTime(DateUtil.date());
        }
    ...
    public R updateStatus(String ids, Integer status) {
        ...
        String subfield = "update 72crm_admin_field set value = '" + a + "' where name = '是否上下架' and batch_id in ( " +
                batchIds.toString() +
                " )";
        int f = Db.update(subfield);
     ...
     * @author zxy
     */
    public R uploadExcel(UploadFile file, Integer repeatHandling, Integer ownerUserId,Long userId) {
        try (ExcelReader reader = ExcelUtil.getReader(FileUtil.file(file.getUploadPath() + "\\" + file.getFileName()))) {
            ...
            if (read.size() > 1) {
                ...
                for (int i = 1; i < read.size(); i++) {
                    ...
                    status = saveAndUpdate(object, userId);
    ...
    public R queryByStatus(BasePageRequest<CrmProduct> basePageRequest) {
        ...
        return adminSceneService.getCrmPageList(basePageRequest,userId);
    }
}
